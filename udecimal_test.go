...
func (s *decimal) Provides_integral_part(t *T) {
	t.Eq(UDecimal(42), UDec.From.MFloat(42.42).Integrals(UDec))
}

func (s *decimal) Provides_fractional_part(t *T) {
	t.Eq(UDecimal(420000), UDec.From.MFloat(42.42).Fractionals(UDec))
	t.Eq(UDecimal(42), UDec.From.MFloat(42.000042).Fractionals(UDec))
}

func (s *decimal) String_uses_set_decimal_mark(t *T) {
	t.Eq("3,20", UDec.From.MFloat(3.2).Str(UDec))
	dec := UDec.New(DEFAULTS, DOT_SEPARATOR)
	t.Eq("3.20", dec.From.MFloat(3.2).Str(dec))
}

func (s *decimal) String_truncates_fractionals_as_set(t *T) {
	dec := UDec.New(DEFAULTS, TWO_FRACTIONALS|DOT_SEPARATOR)
	t.Eq("3.20", dec.From.MFloat(3.2).Str(dec))
...
func (s *decimal) String_pads_suffixing_fractional_zeros(t *T) {
	dec := UDec.New(DEFAULTS, EIGHT_FRACTIONALS|DOT_SEPARATOR)
...
func (s *decimal) String_matches_converted_literal(t *T) {
	dec := UDec.New(DEFAULTS, SIX_FRACTIONALS|DOT_SEPARATOR)
